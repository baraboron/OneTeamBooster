...
POST JSON은 `projectName`, `partner`, `missions`, `boosts`, `impacts`만 사용한다. 사번·이메일·수신자 이름·권한·호출자가 지정한 모델/프롬프트/URL은 전달하지 않는다. 업무명은 자유 입력이므로 사용자가 적은 기밀이나 개인정보까지 자동 제거되는 것은 아니다.
